...
//            e.printStackTrace();
    ...
    public Admin validateCredentials(String userId, String password) {
        try {
//
            Connection con = DBUtils.getConnection();
            String SQL = "select * from user,admin where user.userId like '" + userId + "' and admin.adminId like '" + userId + "' and user.password like '" + password + "'";
            Statement stmt = con.createStatement();
            ResultSet rs = stmt.executeQuery(SQL);
            if (rs.next()) {
                Admin admin = new Admin(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(7), rs.getString(8));
